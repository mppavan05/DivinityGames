...
namespace Mkey
{
    public class SlotPlayer : MonoBehaviour
    {
        ...
        private void LoadCoins()
        {
            ...
            {
                string key = saveCoinsKey;
                // Clear the data in the saveCoinsKey
                PlayerPrefs.DeleteKey(saveCoinsKey);
                string balanceString = DataManager.Instance.playerData.balance;
                // Remove the decimal part, if present
                int decimalIndex = balanceString.IndexOf(".");
                if (decimalIndex != -1)
                {
                    balanceString = balanceString.Substring(0, decimalIndex);
                }
                int newCoinsCount;

                if (int.TryParse(balanceString, out newCoinsCount))
                {
                    // Successfully parsed the string to an integer
                    // Set the new value in PlayerPrefs
                    PlayerPrefs.SetInt(saveCoinsKey, newCoinsCount);
                }
                else
                {
                    // Failed to parse the string to an integer
                    // Handle the error or use a default value
                    Debug.LogError("Failed to convert balanceString to an integer!");
                    newCoinsCount = defCoinsCount;
                }
        ...
        public bool HasMoneyForBet (int totalBet)
        {
             ...
             {
                 return totalBet <= (int)balance;
             }
             else
             {
                 // Failed to parse balanceString to a decimal
                 // Handle the error or return an appropriate default value
                 print("Failed to convert balanceString to a decimal!");
                 return false;
             }
        }
    }
