...
namespace Mkey
{
    ...
    public class SlotController : MonoBehaviour
    {
        ...
        public SpriteRenderer handel;
        public Sprite onHandel;
        public Sprite offHandel;
        ...
        public void ToggleSwitch()
        {
            ...
            handel.sprite = isOn ? onHandel : offHandel; // Set the sprite based on the state

            if (isOn)
                Invoke(nameof(SwitchOff), 0.25f); // Schedule the switch to turn off after the specified duration
        }

        private void SwitchOff()
        {
            handel.sprite = offHandel; // Set the sprite to the "off" sprite
            isOn = false;
        }
        #endregion regular
                ...
                winController.WinSymbolShow(winLineFlashing == WinLineFlashing.Sequenced,
                       (windata) => //linewin
                       {
                           ...
                           foreach (string keyValuePair in keyValuePairs)
                           {
                               ...
                               if (key != "Pay") continue;
                               if (float.TryParse(value, out float payValue))
                               {
                                   winAmount = payValue;
                                   playerWinAmount = winAmount - (winAmount * adminCommssion);
                                   break; // Found the "Pay" value, no need to continue the loop
                               }
                               else
                               {
                                   // Failed to parse the value as a float
                                   // Handle the error or use a default value
                                   Debug.LogError("Failed to convert Pay value to float!");
                                   break;
                               }
                           }
                           SoundManager.Instance.CasinoWinSound();
                           DataManager.Instance.AddAmount((float)(winAmount), DataManager.Instance.gameId, "Slot-Win-" + DataManager.Instance.gameId, "won", (float)(0), 1);
