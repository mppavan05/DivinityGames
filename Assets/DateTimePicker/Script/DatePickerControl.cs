...
public class DatePickerControl : MonoBehaviour {
	...
	void OnEnable () {
        ...
        {
			string birthText = DataManager.Instance.playerData.dob;
			int dayNo = int.Parse(birthText.Split("-")[0]);
			int monthNo = int.Parse(birthText.Split("-")[1]);
			int yearNo = int.Parse(birthText.Split("-")[2]);
